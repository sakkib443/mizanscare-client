...
function ListeningExamPageContent() {
    ...
    // loses every answer the candidate typed. This lives in its own effect, reading through a
        ...
        const t = setInterval(() => {
            try {
                ...
                fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000/api") + "/students/save-draft", {
                    ...
                    body: JSON.stringify({ examId: params.examId, module: "listening", answers: a, setNumber, timeLeft: tl }),
                    keepalive: true,
                }).catch(() => { });
            ...
            <header style={{ backgroundColor: timeLeft <= 60 ? (contrastMode === 'black-on-white' ? '#fde8e8' : '#3a0d0d') : cs.bg, borderBottom: `1px solid ${contrastMode === 'black-on-white' ? '#ccc' : '#555'}`, height: '56px', flexShrink: 0, position: 'sticky', top: 0, zIndex: 50, transition: 'background-color 0.5s ease' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '100%', padding: '0 16px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
                        ...
                        {/* Note-taking icon */}
                        <FaRegStickyNote size={24} color={contrastMode === 'black-on-white' ? '#374151' : cs.text} style={{ cursor: 'pointer' }} onClick={() => setShowNotes(s => !s)} title="Notes" />
                    </div>
                </div>
            </header>
